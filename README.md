...
1. Подключившись к поду приложения: 
```
kubectl exec -it deployment/wikijs -- curl -v http://localhost:3000
```
2. Если есть такая возможность, то подключиться к ноде по ssh и проверить командой:
```
curl -v http://localhost:30123
```
